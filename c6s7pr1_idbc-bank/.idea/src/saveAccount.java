...
public class saveAccount {
    ...
    public String generateAccountNumber()
    {
        ...
        try
        {
         PreparedStatement preparedStatementobj=connection.prepareStatement("select * from savingaccount order by accountid desc ");
    ...
    public void openAccount(String dateOfBirth)
    {
          if(checkage(dateOfBirth))
          {
              try
              {
                  ...
                  PreparedStatement preparedStatementobj= connection.prepareStatement("insert into savingaccount values (?,?,?,?)");
    ...
    public void deposit()
    {
        try
        {
         ...
         PreparedStatement preparedStatementobj= connection.prepareStatement("update savingaccount set amount=amount+? where accountid=?");
    ...
    public  void withdrawal()
    {
        ...
        try {
            PreparedStatement preparedStatementobj= connection.prepareStatement("update savingaccount set amount=amount-? where amount>? and accountid=?");
            Scanner scanner=new Scanner(System.in);
            System.out.println("enter accountid");
    ...
    public  void checkbalance()
    {
        try {
            ...
            System.out.println("enter accountid");
            String accountid= scanner.next();
            PreparedStatement preparedStatementobj= connection.prepareStatement("select * from savingaccount");
            ResultSet resultSetobj= preparedStatementobj.executeQuery();
            while (resultSetobj.next())
            {
        ...
        try
           {
               ...
               while (resultSetobj.next())
               {
                ...
                if(i==2)
               {
                       System.out.println("we cant generate interest because your account is opened less than one year");
               }
               else
               {
                   if(i==0)
    ...
    public void fundstransfer()
    {
        try {
            ...
            System.out.println("enter the acount number to be transfered");
            String accountidToBeTransfered= scanner.next();
            if(!accountid.equals(accountidToBeTransfered)) {
                PreparedStatement preparedStatementobj = connection.prepareStatement("update savingaccount set amount=amount-? where amount>? and accountid=? ");
                ...
                int row = preparedStatementobj.executeUpdate();
                PreparedStatement preparedStatementobj2 = connection.prepareStatement("update savingaccount set amount=amount+? where accountid=?");
            ...
            {
                System.out.println(" sorry not possible same account id");
            }
